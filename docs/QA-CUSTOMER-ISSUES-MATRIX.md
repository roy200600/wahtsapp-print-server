...
| UI/UX baseline | Covered by static QA and customer frontend check | Rubik font, RTL, responsive breakpoints, reduced motion, stable table layout, long-text wrapping, Lucide initialization, service-worker cache versioning, and diagnostics UI are checked in `scripts/qa-smoke.ps1`. `v1.0.56` keeps the dashboard shell hidden until required data loads to reduce blank or broken screens after login. |
...
3. Confirm the customer QA report includes a passed `encryptedPdfDryRun` check.
4. Send a normal PDF and confirm physical output.
5. Send a password-protected PDF and provide `312830714`; confirm physical output.
6. Send a wrong PDF password; confirm no print and system alert to manager plus MY-PC owner.
7. Send JPG/JPEG/PNG and confirm physical output.
8. Send DOC/DOCX and confirm physical output when Office/Word is installed.
9. Send XLS/XLSX and PPT/PPTX only on machines with Office installed and confirm output.
10. Restart the PC during or after a queued job and confirm recovery behavior does not block later jobs.
11. Use a printer with parentheses/spaces in the name and confirm stop-printing/queue cleanup works.
